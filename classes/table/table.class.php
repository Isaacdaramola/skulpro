<?php
    ...
    class table
    {
        ...
        function __construct( $overide_defaults = false , $id = "datatable" ,  $width = "100%" )
        {
            ...
            $this->table_attributes = array(
                "cellpadding"   => "0",
                "cellspacing"   => "0",
                "class"         => "table table-striped table-responsive table-hover table-rounded table-bordered",
                "id"            => "datatable",
                "width"         => "100%"
            );
        ...
        # Alias to heading
        function headings($array , array $to_unset = null)
        {
          if( is_array( $to_unset )){
            foreach ($to_unset as $key => $v ) {
              unset( $array[$key] );
        ...
        function addRow($array , array $to_unset = null )
        {
          // if( is_array( $to_unset )){
          //   foreach ($to_unset as $key => $v ) {
          //     unset( $array[$key] );
        ...
        function render()
        {
            ...
            if (!empty($this->table_heading))
            {
                ...
                foreach($this->table_heading as $th)
                {
                    ...
                    $output .= "\t\t<th " . $implode . ">" . strtoupper( str_replace( array('-' , '_') , ' ' , $value ) ) . "</th>\n";
                }
                $output .= "\t</thead>\n";
            }
